...
- **Persistencia por modelo:** se guarda el `LayoutJson` extraído **+ una RI de referencia (PDF) en `FileData`**. El **PDF fuente NO se persiste** (se usa solo transitoriamente para extraer). Hay opción de **actualizar** (re-subir el fuente → re-extraer → regenerar `LayoutJson` + RI de referencia).
...
- `FileName`, `ContentType`, `FileData` (byte[]) → **RI de referencia generada** (PDF reconstruido con datos representativos) para vista previa/registro. **El PDF fuente NO se persiste** (transitorio, solo para extraer). Se **elimina** `EcfTypeId` único (se reemplaza por join). `FileUrl` eliminado.
...
- Persiste por modelo: `LayoutJson` (para renderizar) **+ una RI de referencia** en `FileData`. El **fuente no se persiste**.
- Las RI reales de cada comprobante se generan **on-demand** (preview/descarga) desde `LayoutJson`. El ZIP se arma temporalmente en `wwwroot` como los ZIP del Paso 4.
...
- `POST /print-templates` (multipart: `pdfFile`, `clientGuidId`, `name`, `ecfTypeCodes[]`) → corre extractor (`LayoutJson`), **genera la RI de referencia** (`FileData`), **descarta el fuente**, estado `Extracted` (+ warnings).
- `GET /print-templates/{clientGuidId}` → lista modelos del cliente (tipos, estado, warnings).
- `GET /print-templates/{templateGuidId}/preview` → devuelve la **RI de referencia** (`FileData`) — es la vista previa de confirmación.
- `PUT /print-templates/{templateGuidId}` (multipart opcional) → renombrar / reasignar tipos / `Confirm`, **y/o re-subir un nuevo `pdfFile`** para **re-extraer** (regenera `LayoutJson` + RI de referencia).

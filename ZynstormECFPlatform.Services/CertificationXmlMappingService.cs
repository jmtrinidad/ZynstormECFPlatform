...
public class CertificationXmlMappingService : ICertificationXmlMappingService
{
    ...
    public EcfInvoiceRequestDto PrepareSimulationStep4Xml(EcfInvoiceRequestDto dto)
    {
        // Exclusivo para certificación paso 4: mantener datos del DTO sin inventar campos
        // y asegurar consistencia básica de totales para validación XSD/transmisión.
        var result = dto;
        var items = result.ECF.DetallesItems.Item ?? [];
        if (items.Count == 0) return result;

        var gravado = items.Where(i => i.IndicadorFacturacion == "1").Sum(i => i.CantidadItem * i.PrecioUnitarioItem);
        var exento = items.Where(i => i.IndicadorFacturacion == "4").Sum(i => i.CantidadItem * i.PrecioUnitarioItem);
        var total = items.Sum(i => (i.CantidadItem * i.PrecioUnitarioItem) - (i.DescuentoMonto ?? 0m) + (i.RecargoMonto ?? 0m));

        result.ECF.Encabezado.Totales.MontoGravadoTotal ??= gravado;
        result.ECF.Encabezado.Totales.MontoExento ??= exento;
        result.ECF.Encabezado.Totales.MontoTotal ??= total;
        result.ECF.Encabezado.Totales.TotalITBIS ??= 0m;
    ...
    {
        if (!int.TryParse(dto.ECF.Encabezado.IdDoc.TipoeCF, out var ecfType))
            return dto;

        var rule = CertificationTypeMatrix.TryGetValue(ecfType, out var foundRule)
            ? foundRule
            : new CertificationTypeRule();

        foreach (var item in dto.ECF.DetallesItems.Item)
        {
            if (rule.AllowDescuentoMonto &&
                !item.DescuentoMonto.HasValue &&
                item.TablaSubDescuento?.SubDescuento?.Any(s => s.MontoSubDescuento.HasValue) == true)
            {
                item.DescuentoMonto = item.TablaSubDescuento.SubDescuento
                    .Where(s => s.MontoSubDescuento.HasValue)
                    .Sum(s => s.MontoSubDescuento!.Value);
            }

            if (rule.AllowRecargoMonto &&
                !item.RecargoMonto.HasValue &&
                item.TablaSubRecargo?.SubRecargo?.Any(s => s.MontoSubRecargo.HasValue) == true)
            {
                item.RecargoMonto = item.TablaSubRecargo.SubRecargo
                    .Where(s => s.MontoSubRecargo.HasValue)
                    .Sum(s => s.MontoSubRecargo!.Value);
            }

            if (!rule.AllowDescuentoMonto) item.DescuentoMonto = null;
            if (!rule.AllowTablaSubDescuento) item.TablaSubDescuento = null;
            if (!rule.AllowRecargoMonto) item.RecargoMonto = null;
            if (!rule.AllowTablaSubRecargo) item.TablaSubRecargo = null;
        }
